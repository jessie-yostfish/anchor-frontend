/*
  # Fix Legal Content Table Schema

  ## Changes
  
  1. Drop existing legal_content table with incorrect schema
  2. Recreate legal_content table with correct columns:
     - description, full_content, plain_language, related_topics, external_link, subsections, view_count
  3. Recreate all indexes and triggers
  4. Insert sample legal content data
  
  ## Schema
  The corrected schema matches the frontend expectations with proper columns for detailed legal information.
*/

-- Drop existing table
DROP TABLE IF EXISTS legal_content CASCADE;

-- Create legal_content table with correct schema
CREATE TABLE legal_content (
  ...
  CONSTRAINT valid_category CHECK (category IN ('statutes', 'rights', 'procedures', 'forms'))
);

-- Create indexes
CREATE INDEX idx_legal_content_title ON legal_content(title);
CREATE INDEX idx_legal_content_category ON legal_content(category);
CREATE INDEX idx_legal_content_search_vector ON legal_content USING GIN(search_vector);
CREATE INDEX idx_legal_content_related ON legal_content USING GIN(related_topics);
...
('California Welfare and Institutions Code (WIC)', 'statutes', 'The primary California law governing dependency proceedings', 
...
('Your Rights at the Detention Hearing', 'rights', 'What you are entitled to at the first court hearing',
...
('Right to an Attorney', 'rights', 'Your right to free legal representation in dependency court',
...
('Parental Rights in Dependency Court', 'rights', 'Your fundamental rights throughout the dependency process',
...
('Timeline for Court Hearings', 'procedures', 'When each hearing must occur in a dependency case',
...
('Understanding the Case Plan', 'procedures', 'What a case plan is and why it matters',
...
('Grounds for Dependency (WIC Section 300)', 'statutes', 'The legal reasons a child can be declared a dependent of the court',
